...
class Config():
    def __init__(self, parser, dataset_name, load=True):
        ...
        # parser.add_argument('--dataset_name', default='d7class', type=str,
        ...
        parser.add_argument('--filename_wordvec', default='wikipedia-pubmed-and-PMC-w2v.txt', 
        ...
        parser.add_argument('--use_crf', action='store_false', 
                    help='whether to not use crf optimization layer')
        ...
        self.filename_wordvec = os.path.join('embeddings/word2vec/', 
                                            self.filename_wordvec)
        self.dir_output = os.path.join('results', dataset_name)
        ...
        if dataset_name == 'pubmed-20k':
            self.filename_dev = "data/PubMed_20k_RCT/dev_clean.txt"
            self.filename_test = "test_folder/temp/abstract_temp.txt" #changed this
        ...
        elif dataset_name == 'dim':
            self.filename_dev = "data/Dimitris_data/dev_clean.txt"
            self.filename_test = "test_folder/temp/abstract_temp.txt" #changed this
            self.filename_train = "data/Dimitris_data/train_clean_nocommons.txt"
            self.filename_words = "data/Dimitris_data/words.txt"
            self.filename_tags = "data/Dimitris_data/tags.txt"
            self.filename_chars = "data/Dimitris_data/chars.txt"
            self.filename_wordvec_trimmed = os.path.join('data/Dimitris_data', self.filename_wordvec_trimmed)
        elif dataset_name == 'nicta':
        ...
        elif dataset_name == 'd7class':
            self.filename_dev = "data/D7class_data/dev_clean.txt"
            self.filename_test = "test_folder/temp/abstract_temp.txt" #changed this
            self.filename_train = "data/D7class_data/train_clean.txt"
            self.filename_words = "data/D7class_data/words.txt"
            self.filename_tags = "data/D7class_data/tags.txt"
            self.filename_chars = "data/D7class_data/chars.txt"
            self.filename_wordvec_trimmed = os.path.join('data/D7class_data', self.filename_wordvec_trimmed)
        else:
    ...
    def load(self):
        ...
        # self.embeddings = np_to_tfrecords(self.embeddings_temp, None, 'embeddings', verbose=True)
