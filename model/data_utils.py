...
def format_sentence(sentences):
    sentences_split = filter(None, re.split('(?<!\w\.\w.)(?<![A-Z]\.)(?<![A-Z][a-z]\.)(?<=\.|\?) ', sentences))
...
def is_url(word):
    if re.search('www', word) or re.search('http', word):
        print("word is url")
        return True
    else:
...
def is_margin(word):
    if re.search(r'[0-9-0-9]', word):
        print("word is margin")
        return True
    else:
        print("word is not margin")
        return False
